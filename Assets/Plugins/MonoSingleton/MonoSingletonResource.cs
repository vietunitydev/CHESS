...
namespace Metarush
{
    ...
    /// </summary>
    public abstract class MonoSingletonResource<T> : MonoBehaviour where T : MonoBehaviour
    {
        private const string PrefabPath = "Prefab/MonoSingleton/";
        ...
        ///to make sure we has instance. And don't call Instance at Disable() or Destroy(), will lead to memory leak !!</summary>
        public static T Instance
        {
            get
            {
                ...
                {
                    Debug.LogWarning(
                        "Call Instance, but Application is not running, are you call to any Singleton at OnDestroy?");
                    return null;
                }

                // dont exist, find load from resource
                if (s_instance == null)
                {
                    var path = Path.Combine(PrefabPath, typeof(T).Name);
                    var prefabs = Resources.Load<T>(path);
                    s_instance = Instantiate(prefabs);
                }
        ...
        protected virtual bool IsDontDestroyThisObject
        {
            get { return dontDestroyThisObject; }
        }
        ...
        protected void Awake()
        {
            ...
            if (s_instance != null && s_instance.GetInstanceID() != this.GetInstanceID())
            {
                ...
                Debug.LogFormat("An instance of [{0}] already exist : [{1}], So destroy this instance : [{2}]!!",
                    typeof(T).ToString(), s_instance.gameObject.name, name);
                Destroy(gameObject);
            }
